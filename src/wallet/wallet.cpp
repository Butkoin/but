...
bool CWallet::AddKeyPubKeyWithDB(WalletBatch &batch, const CKey& secret, const CPubKey &pubkey)
{
    ...
    if (!CCryptoKeyStore::AddKeyPubKey(secret, pubkey)) {
        if (needsDB) encrypted_batch = NULL;
        return false;
        return false;
    }
    if (needsDB) encrypted_batch = NULL;
...
bool CWallet::Verify()
{
    ...
    for (const std::string& walletFile : gArgs.GetArgs("-wallet")) {
        ...
        std::string strError;
        if (!WalletBatch::VerifyEnvironment(walletFile, GetDataDir().string(), strError)) {
        ...
        if (gArgs.GetBoolArg("-salvagewallet", false)) {
            ...
            if (!WalletBatch::Recover(walletFile, (void *)&dummyWallet, WalletBatch::RecoverKeysOnlyFilter, backup_filename)) {
                return false;
            }
        }

        std::string strWarning;
        bool dbV = WalletBatch::VerifyDatabaseFile(walletFile, GetDataDir().string(), strWarning, strError);
        if (!strWarning.empty()) {
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !WalletBatch(*database).WritePurpose(CBitcoinAddress(address).ToString(), strPurpose))
        return false;
    return WalletBatch(*database).WriteName(CBitcoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBook(const CTxDestination& address)
{
    ...
    WalletBatch(*database).ErasePurpose(CBitcoinAddress(address).ToString());
    return WalletBatch(*database).EraseName(CBitcoinAddress(address).ToString());
}
...
{
    if (!WalletBatch(*database).WriteDefaultKey(vchPubKey))
        return false;
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return WalletBatch(*database).WriteDestData(CBitcoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return WalletBatch(*database).EraseDestData(CBitcoinAddress(dest).ToString(), key);
}
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (gArgs.GetBoolArg("-zapwallettxes", false)) {
        ...
        std::unique_ptr<WalletDatabase> database(new WalletDatabase(&bitdb, walletFile));
        CWallet *tempWallet = new CWallet(std::move(database));
    ...
    bool fFirstRun = true;
    std::unique_ptr<WalletDatabase> database(new WalletDatabase(&bitdb, walletFile));
    CWallet *walletInstance = new CWallet(std::move(database));
